...
let getProductsAndCategories(conn: SqlConnection) = task {
    ...
    let getOrdinalByTable = AdventureWorks.Readers.getOrdinalByEntity<SalesLT.Product * SalesLT.ProductCategory> reader

    let product = SalesLT.ProductDataReader(reader, getOrdinalByTable.["Product"])
    let category = SalesLT.ProductCategoryDataReader(reader, getOrdinalByTable.["ProductCategory"])
